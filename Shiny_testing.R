...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      radioButtons("Grp_prop", label = h3("Select data by:"), 
    ...
    mainPanel(
      tabsetPanel(type="tab",
                  ...
                  tabPanel("Group proportions split into gender", verbatimTextOutput("summ"),
                  )
      )
    )
  ) 
...
server <- function(input, output) {
  ...
  output$summ = renderPrint({prop_func(nhanes2, 'agegroup', 'Smoked_100')})
}
