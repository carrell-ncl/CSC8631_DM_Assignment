...
colnames(nhanes2)
setwd('/Users/carrell/')
t = nhanes2[['Systolic_BP1']]
s = nhanes2[['Systolic_BP2']]
cor(t, s, use = 'complete.obs')
#Import libraries
library(ggplot2)
library(dplyr)
library(utils)
...
class(nhanes2$Gender)
#Rename columns - created a new datas et incase I need to revert back to original
nhanes2 = nhanes %>% dplyr::rename(Gender = RIAGENDR, Age = RIDAGEYR, Race = RIDRETH1, 
...
my_func(nhanes2, 'Gender', 'Systolic_BP1')
...
t = nhanes2[['Diastolic_BP1']]
s = nhanes2[['Diastolic_BP2']]
cor(t, s,use = 'complete.obs')

class(nhanes2)

cor(nhanes2$Diastolic_BP1, nhanes2$Diastolic_BP2, use = 'complete.obs')
