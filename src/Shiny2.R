    ...
    sidebarPanel(
      ...
      selectInput(inputId = "x", label = "Scatter - Variable in X axis:",
                  choices = c('Age', 'Systolic_BP1', 'Systolic_BP2',
                              'Diastolic_BP1', 'Diastolic_BP2', 'BMXWT', 'BMXHT', 'BMXBMI', 'BMXWAIST', 'BMXWT'),
      ...
      selectInput(inputId = "x2", label = "Box - Variable in X axis:",
                  choices = c('agegroup', "Alcohol_Year", "Smoked_100", "Race", "EducationX", "Marital_Status",
                              'Household_Size', 'Income_to_Pov', 'Income_Group'),
    ...
    mainPanel(
      ...
      tabPanel("Group table with proportions", radioButtons("Grp_prop", label = h3("Select data by group:"), 
                                                         choices = list("Age Group" = "agegroup",
                                                                        "Race" = "Race", "Education" = "EducationX", 
               ...
               radioButtons("analyse_prop", label = h3("Select data by:"), 
                            choices = list("Smoked 100" = "Smoked_100", "Alcohol Year" = "Alcohol_Year",
                                           "Education" = "EducationX", "Race" = "Race"),
                            selected = "Smoked_100"),DT::dataTableOutput("prop"))),
...
server <- function(input, output) {
  ...
  output$prop = DT::renderDataTable({prop_func(nhanes2, input$Grp_prop, input$analyse_prop)})
